...
DROP TABLE emp_test; 
DROP TABLE dept_test; -- 자식이 있는경우 못주거.. 자식먼저 지워야 DROP가능

CREATE TABLE dept_test(
    deptno NUMBER(2), /* PRIMARY KEY --> UNIQUE 제약 --> 인덱스생성X */
    dname VARCHAR2(14),
    loc VARCHAR2(13)
);
...
CREATE TABLE emp_test(
    empno NUMBER(4),
    ename VARCHAR2(10),
    --dept_test.deptno컬럼에 인덱스가 없기 때문에 정상적으로 
    --fk제약을 생성할 수 없다.
    deptno NUMBER(2) REFERENCES dept_test(deptno)
);
...
-- 즉, 자식 테이블에서 참조하는 데이터가 없어야 부모테이블의 데이터를 삭제 가능하다.
DELETE dept_test WHERE deptno=1; 

--FK제약 옵션
--default : 데이터 입력/삭제 시 순차적으로 처리해줘야 fk제약을
--          위배하지 않는다.
...
--sal컬럼은 CHECK 제약 조건에 의해 0이거나, 0보다 큰 값만 입력이 가능하다.
...
SELECT *
FROM customer
WHERE 1=2; --절대 참일 수 없는 조건을 주고 테이블을 생성한다!! ex) 1=2, 1!=1
...
--3. emp테이블의 deptno컬럼으로 dept테이블의 deptno컬럼을
--참조하는 dk제약을 테이블 변경 ddl을 통해 생성
--emp --> dept(deptno)
